...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.categories.store') }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" class="form-control" id="add-category-form-title" placeholder="Заголовок"
                   value="{{ old('title') }}">
            <label for="add-category-form-url">Слаг Url</label>
            <input type="text" name="url" class="form-control" id="add-category-form-url" placeholder="Url"
                   value="@if(old('url')) {{ old('url') }}@else {{ fake()->slug() }} @endif">
        </div>
        <div class="form-group">
            <label for="add-category-form-description">Описание</label>
            <textarea name="description" class="form-control" id="add-category-form-description"
